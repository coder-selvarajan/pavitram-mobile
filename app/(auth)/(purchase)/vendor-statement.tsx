...
export default function VendorStatementScreen() {
      ...
      isAdmin ? (
        <View className="flex-row items-center gap-1.5">
          <TouchableOpacity
            onPress={() =>
              router.push(
                `/(auth)/(purchase)/payment-edit?projectId=${projectId}&vendorId=${vendorId}&paymentId=new`,
              )
            }
  ...
  const handleItemPress = (item: StatementItem) => {
    if (item.type === 'bill') {
      router.push(
        `/(auth)/(purchase)/bill-edit?projectId=${projectId}&vendorId=${vendorId}&billId=${item.data.id}`,
      );
    } else if (isAdmin) {
      router.push(
        `/(auth)/(purchase)/payment-edit?projectId=${projectId}&vendorId=${vendorId}&paymentId=${item.data.id}`,
      );
    }
  };
