...
import type { Project, Customer, SalesPayment, PaymentMethod } from '../../../types';

export default function SalesPaymentEditScreen() {
  const { projectId, customerId, paymentId } = useLocalSearchParams<{
    projectId: string;
    customerId: string;
    paymentId: string;
  }>();
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const paymentPromise = !isNew && paymentId
        ? supabase.from('payments_sales').select('*').eq('id', paymentId).single()
        : null;

      const [projectRes, customerRes, allCustomersRes, methodsRes, paymentRes] = await Promise.all([
        projectPromise,
        customerPromise,
        allCustomersPromise,
        methodsPromise,
      ...
      if (paymentRes?.data) {
        const payment = paymentRes.data as SalesPayment;
        setDate(new Date(payment.date));
        setAmount(String(payment.amount));
        setPaymentMethodId(payment.payment_method_id);
        setDescription(payment.description ?? '');
        setSelectedCustomerId(payment.customer_id);
        if (!customerId && payment.customer_id) {
          const { data: c } = await supabase
            .from('customers').select('*').eq('id', payment.customer_id).single();
          if (c) setCustomer(c);
        }
      }
    } catch (err) {
      console.error('Error fetching sales payment data:', err);
      Alert.alert('Error', 'Failed to load payment data');
    } finally {
      setLoading(false);
    }
  }, [projectId, customerId, paymentId, isNew]);
      ...
      [
        { text: 'Cancel', style: 'cancel' },
        {
          ...
          onPress: async () => {
            setSaving(true);
            try {
              const { error } = await supabase.from('payments_sales').delete().eq('id', paymentId);
